    ...
    def forward(self, src: Tensor, mask: Optional[Tensor] = None, src_key_padding_mask: Optional[Tensor] = None, prompt_embeds: Optional[Tensor] = None) -> Tensor:
        ...
        for mod in self.layers:
            if prompt_embeds is not None:
                ...
                output = mod(output, src_mask=mask, src_key_padding_mask=src_key_padding_mask, prompt_embeds=prompt_embeds)
            else:
...
def multi_head_attention_forward(
    ...
    if prompt_embeds is not None:
        ...
        k = torch.cat([prompt_embeds, k], dim=-3)
        v = torch.cat([prompt_embeds, v], dim=-3)
    ...
    if prompt_embeds is not None:
        ...
        prompt_padding = torch.zeros(key_padding_mask.shape[0], prompt_embeds.shape[0], device=k.device)
